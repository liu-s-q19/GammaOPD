...
class RLHFDataset(Dataset):
    ...
    def _load_parquet_dataset(self, parquet_file: str) -> datasets.Dataset:
        try:
            ...
            if not self._should_fallback_to_pyarrow(exc):
                raise

            logger.warning(
                "Falling back to direct pyarrow parquet loading for %s due to incompatible nested parquet conversion: %s",
                parquet_file,
                exc,
            )
    ...
    @staticmethod
    def _should_fallback_to_pyarrow(exc: Exception) -> bool:
        current = exc
        while current is not None:
            if isinstance(current, NotImplementedError):
                return True
            if "Nested data conversions not implemented for chunked array outputs" in str(current):
                return True

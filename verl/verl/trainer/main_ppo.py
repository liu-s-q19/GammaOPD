...
def _save_hydra_snapshot_to_ckpt_dir(config) -> None:
    """Persist launch configs into ckpt root for reproducibility."""
    ckpt_dir = Path(str(config.trainer.default_local_dir))
    ckpt_dir.mkdir(parents=True, exist_ok=True)

    target_hydra_dir = ckpt_dir / ".hydra"
    target_hydra_dir.mkdir(parents=True, exist_ok=True)

    # Always store a resolved snapshot under ckpt root.
    OmegaConf.save(config, ckpt_dir / "resolved_config.yaml", resolve=True)

    # Best-effort copy of Hydra runtime metadata.
    try:
        runtime_output_dir = Path(HydraConfig.get().runtime.output_dir)
        source_hydra_dir = runtime_output_dir / ".hydra"
        for name in ("config.yaml", "hydra.yaml", "overrides.yaml"):
            src = source_hydra_dir / name
            if src.exists():
                shutil.copy2(src, target_hydra_dir / name)
    except Exception as exc:
        print(f"[WARN] Failed to copy hydra runtime metadata to {target_hydra_dir}: {exc}")
...
def run_ppo(config, task_runner_class=None) -> None:
    ...
    """
    _save_hydra_snapshot_to_ckpt_dir(config)

...
from verl.trainer.ppo.metric_utils import (
    ...
    flatten_legacy_validation_metrics,
    flatten_validation_metrics,
    process_validation_metrics,
    summarize_benchmark_accuracy_metrics,
)
...
def _normalize_config_path(path: str) -> str:
    """Normalize a configured dataset path for equality checks."""
    return os.path.abspath(os.path.expanduser(path))


def should_run_multi_benchmark_validation(config, global_steps: int) -> bool:
    """Return whether extra benchmark validation should run at the current step."""
    if config is None:
        return False
    if not config.get("enabled", False):
        return False
    if global_steps < config.get("start_step", 50):
        return False
    return len(config.get("benchmarks", [])) > 0


def _empty_validation_result() -> tuple[dict[str, dict[str, dict[str, float]]], dict[str, float]]:
    """Return an empty validation result matching `_validate_dataloader`'s contract."""
    return {}, {}
...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        self.extra_val_dataloaders = self._create_extra_val_dataloaders(create_rl_dataset, num_workers)
    ...
    def _create_extra_val_dataloaders(self, create_rl_dataset, num_workers: int) -> dict[str, StatefulDataLoader]:
        """Create named validation dataloaders for optional multi-benchmark evaluation."""
        multi_benchmark_cfg = self.config.trainer.get("multi_benchmark_val", None)
        if (
            not multi_benchmark_cfg
            or not multi_benchmark_cfg.get("enabled", False)
            or len(multi_benchmark_cfg.get("benchmarks", [])) == 0
        ):
            return {}

        val_batch_size = self.config.data.val_batch_size
        val_dataloaders = {}
        for benchmark in multi_benchmark_cfg.benchmarks:
            val_dataset = create_rl_dataset(
                benchmark.path,
                self.config.data,
                self.tokenizer,
            ...
            benchmark_val_batch_size = val_batch_size if val_batch_size is not None else len(val_dataset)
            val_dataloaders[benchmark.name] = StatefulDataLoader(
                dataset=val_dataset,
                batch_size=benchmark_val_batch_size,
                num_workers=num_workers,
                shuffle=self.config.data.get("validation_shuffle", True),
                drop_last=False,
    ...
    def _resolve_primary_validation_benchmark_name(self) -> str:
        """Resolve the benchmark namespace for the primary validation dataset."""
        multi_benchmark_cfg = self.config.trainer.get("multi_benchmark_val", None)
        if not multi_benchmark_cfg or len(multi_benchmark_cfg.get("benchmarks", [])) == 0:
            return "primary"

        benchmark_names = [benchmark.name for benchmark in multi_benchmark_cfg.benchmarks]

        val_files = self.config.data.val_files
        if not isinstance(val_files, (list, tuple, ListConfig)):
            val_files = [val_files]
        if len(val_files) != 1:
            return "primary"

        primary_path = _normalize_config_path(val_files[0])
        for benchmark in multi_benchmark_cfg.benchmarks:
            if _normalize_config_path(benchmark.path) == primary_path:
    ...
    def _validate_dataloader(
        ...
        if dump_generations and val_data_dir:
            dump_path = val_data_dir if benchmark_name == "primary" else os.path.join(val_data_dir, benchmark_name)
            self._dump_generations(
        ...
        if len(sample_turns) > 0:
            ...
            aux_metric_dict[f"val-aux/{benchmark_name}/num_turns/min"] = sample_turns.min()
            aux_metric_dict[f"val-aux/{benchmark_name}/num_turns/max"] = sample_turns.max()
            aux_metric_dict[f"val-aux/{benchmark_name}/num_turns/mean"] = sample_turns.mean()

        return data_src2var2metric2val, aux_metric_dict

    def _validate(self):
        benchmark2metrics = {}
        aux_metric_dict = {}

        primary_benchmark_name = self._resolve_primary_validation_benchmark_name()
        primary_metrics, primary_aux_metrics = self._validate_dataloader(
            self.val_dataloader,
            benchmark_name=primary_benchmark_name,
            log_generations=True,
            dump_generations=True,
        )
        benchmark2metrics[primary_benchmark_name] = primary_metrics
        aux_metric_dict.update(primary_aux_metrics)

        multi_benchmark_cfg = self.config.trainer.get("multi_benchmark_val", None)
        if should_run_multi_benchmark_validation(multi_benchmark_cfg, self.global_steps):
            for benchmark_name, val_dataloader in self.extra_val_dataloaders.items():
                if benchmark_name == primary_benchmark_name:
                    continue
                benchmark_metrics, benchmark_aux_metrics = self._validate_dataloader(
                    val_dataloader,
                    benchmark_name=benchmark_name,
                )
                benchmark2metrics[benchmark_name] = benchmark_metrics
                aux_metric_dict.update(benchmark_aux_metrics)

        metric_dict = flatten_validation_metrics(benchmark2metrics)
        metric_dict.update(flatten_legacy_validation_metrics(primary_metrics))
        metric_dict.update(
            summarize_benchmark_accuracy_metrics(
                benchmark2metrics,
                include_summary=should_run_multi_benchmark_validation(multi_benchmark_cfg, self.global_steps),
            )
        )
        metric_dict.update(aux_metric_dict)
        return metric_dict

    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                if not isinstance(var_vals[0], (int, float, bool, np.number, np.bool_)):
                    continue
...
def flatten_validation_metrics(
    benchmark2data_src2var2metric2val: dict[str, dict[str, dict[str, dict[str, float]]]],
) -> dict[str, float]:
    """Flatten benchmark-scoped validation metrics for logging.

    Args:
        benchmark2data_src2var2metric2val: Nested metrics keyed by benchmark name, then
            data source, variable name, and metric name.

    Returns:
        Flat metric dictionary suitable for tracking backends.
    """
    metric_dict = {}
    for benchmark_name, data_src2var2metric2val in benchmark2data_src2var2metric2val.items():
        include_data_source = len(data_src2var2metric2val) > 1
            ...
            for var_name, metric2val in var2metric2val.items():
                ...
                for metric_name, metric_val in metric2val.items():
                    ...
                    if include_data_source:
                        metric_key = f"{metric_sec}/{benchmark_name}/{data_source}/{var_name}/{metric_name}"
                    else:
                        metric_key = f"{metric_sec}/{benchmark_name}/{var_name}/{metric_name}"
                    metric_dict[metric_key] = metric_val

    return metric_dict


def flatten_legacy_validation_metrics(
    data_src2var2metric2val: dict[str, dict[str, dict[str, float]]],
) -> dict[str, float]:
    """Flatten one validation pass using the pre-benchmark metric naming scheme."""
    metric_dict = {}
    for data_source, var2metric2val in data_src2var2metric2val.items():
        core_var = "acc" if "acc" in var2metric2val else "reward"
        for var_name, metric2val in var2metric2val.items():
...
def summarize_benchmark_accuracy_metrics(
    benchmark2data_src2var2metric2val: dict[str, dict[str, dict[str, dict[str, float]]]],
    include_summary: bool,
) -> dict[str, float]:
    """Create compact averaged accuracy summaries.

    This summary is intended for convenient TensorBoard scalar inspection on top of the
    more detailed `val-core/...` metric tree.
    """
    if not include_summary:
        return {}

    def _extract_benchmark_acc(data_src2var2metric2val: dict[str, dict[str, dict[str, float]]]) -> float | None:
        source_acc_vals = []
        for var2metric2val in data_src2var2metric2val.values():
            if "acc" not in var2metric2val:
                continue
            acc_metrics = var2metric2val["acc"]
            mean_metric_names = [name for name in acc_metrics.keys() if name.startswith("mean@")]
            if not mean_metric_names:
                continue
            metric_name = max(mean_metric_names, key=lambda name: int(name.split("@")[-1].split("/")[0]))
            source_acc_vals.append(acc_metrics[metric_name])

        if not source_acc_vals:
            return None
        return float(np.mean(source_acc_vals))

    all_acc_vals = []

    for benchmark_name, data_src2var2metric2val in benchmark2data_src2var2metric2val.items():
        benchmark_acc = _extract_benchmark_acc(data_src2var2metric2val)
        if benchmark_acc is None:
            continue
        all_acc_vals.append(benchmark_acc)

    if all_acc_vals:
        return {"val-summary/acc_mean": float(np.mean(all_acc_vals))}

...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        use_local_group_baseline = self.config.actor.policy_loss.use_local_group_baseline
        use_suffix_group_baseline = self.config.actor.policy_loss.use_suffix_group_baseline
        use_aopd = self.config.actor.policy_loss.use_aopd
        use_any_group_baseline = use_local_group_baseline or use_suffix_group_baseline or use_aopd
        local_group_topk = (
            self.config.actor.policy_loss.aopd_teacher_topk if use_aopd else self.config.actor.policy_loss.local_group_topk
        )
        data.meta_info["compute_local_group_stats"] = use_any_group_baseline
        data.meta_info["gather_local_group_ref_log_probs"] = False
        data.meta_info["local_group_topk"] = local_group_topk
        # perform recompute log_prob
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output = self.actor.compute_log_prob(data=data, calculate_entropy=True)

            if use_any_group_baseline:
                output, entropys, local_topk_ids, local_student_log_probs = output
                output = DataProto.from_dict(
                    tensors={
                        "old_log_probs": output,
                        "entropys": entropys,
                        "opd_local_topk_ids": local_topk_ids,
                        "opd_local_student_log_probs": local_student_log_probs,
                    },
                    meta_info={"temperature": self.config.rollout.temperature},
                )
            else:
                output, entropys = output
                output = DataProto.from_dict(
    ...
    def compute_ref_log_prob(self, data: DataProto):
        if self._is_lora:
            ...
            if "opd_local_student_log_probs" in data.batch.keys():
                tensors["opd_local_ref_log_probs"] = data.batch["opd_local_student_log_probs"]
        ...
        data.meta_info["gather_local_group_ref_log_probs"] = "opd_local_topk_ids" in data.batch.keys()
        data.meta_info["compute_local_group_stats"] = False
        data.meta_info["local_group_topk"] = (
            self.config.actor.policy_loss.aopd_teacher_topk
            if self.config.actor.policy_loss.use_aopd
            else self.config.actor.policy_loss.local_group_topk
        )
        with self.ulysses_sharding_manager:
            data = data.to("cpu")  # data will to device with each micro batch on ref.compute_log_prob
            output = self.ref_policy.compute_log_prob(data=data, calculate_entropy=False)
            if data.meta_info["gather_local_group_ref_log_probs"]:
                output, _, _, local_ref_log_probs = output
                tensors = {"ref_log_prob": output, "opd_local_ref_log_probs": local_ref_log_probs}
            else:
                output, _ = output
